...
import { ensureNotificationPermissionsAsync, scheduleNewArticleNotification } from '../notifications';
...
describe('ensureNotificationPermissionsAsync', () => {
  ...
  it('chưa có quyền nhưng canAskAgain → gọi requestPermissionsAsync và trả kết quả đó', async () => {
...
describe('scheduleNewArticleNotification', () => {
  it('có quyền → gọi scheduleNotificationAsync với trigger:null, trả true', async () => {
    mockGetPermissionsAsync.mockResolvedValueOnce({ granted: true, canAskAgain: true });
  ...
  it('không có quyền → trả false, KHÔNG gọi scheduleNotificationAsync', async () => {
    mockGetPermissionsAsync.mockResolvedValueOnce({ granted: false, canAskAgain: false });
